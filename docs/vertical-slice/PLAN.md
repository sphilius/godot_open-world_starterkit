...
| **M6** ✅ | 06 | AI: `CombatDirector` (tokens, flank ring), `EnemyCombatController` (humanoid), Grunt and Brute scenes, telegraph glint VFX, Gatekeeper boss (2 phases), executions and the boss bar. Placeholder humanoid rigs; a sparring yard and the Gatekeeper sit at the path's end until M8 builds the courtyard and sanctum | `scripts/ai/`, `scenes/mobs/` | M5, M2 characters (or placeholder humanoid) | 6–10 h | 2–3 wk |

...
| **M6** ✅ | 06 | AI: `CombatDirector` (tokens, flank ring), `EnemyCombatController` (humanoid), Grunt and Brute scenes, telegraph glint VFX, Gatekeeper boss (2 phases), executions and the boss bar. Placeholder humanoid rigs | `scripts/ai/`, `scenes/mobs/` | M5, M2 characters (or placeholder humanoid) | 6–10 h | 2–3 wk |
...
| **M9** (M9a ✅ loop) | 09 | **M9a ✅**: `GameManager` (a scene node, not an autoload), `CheckpointShrine`, title, pause and victory screens, death slow motion and fades, per-beat lighting (D9). **M9b**: audio (foley, impacts, parry, music), `CameraTrauma`, `CombatHUD`, VFX (hit sparks, parry flash, glints, blood or ink decals) | `scripts/audio/`, `scripts/ui/`, `scripts/game/` | M6–M8, **audio purchases** | 5–8 h | 2–3 wk |
